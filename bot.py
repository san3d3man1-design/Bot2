...
async def msg_handler(message: types.Message):
    ...
    if txt.startswith("/start join_"):
        ...
        async with pool.acquire() as conn:
            await conn.execute("UPDATE deals SET buyer_id=$1 WHERE deal_token=$2", uid, token)
            deal = await conn.fetchrow("SELECT amount,description,payment_token FROM deals WHERE deal_token=$1", token)
        if deal:
